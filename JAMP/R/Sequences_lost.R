#
Sequences_lost <- function(Reads_in=NA, Reads_out, Sample_names, out="", abundance=T, rel=F, col=c("#b2df8a", "#fb9a99", "#33a02c",  "#fb9a99")){
...
if(out!=""){
pdf(out, height=c(length(Reads_in)+3)/4, width=8)
}

par(mar=c(3,7,1,0))
x <- barplot(data, col=col, border=0, horiz=T, xlim=c(0,max(data[1,]+data[2,])*1.1), ylim=c(0.042*length(Reads_in), 1.17*length(Reads_in)))
