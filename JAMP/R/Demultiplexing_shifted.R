...
glumanda <- barcodes[which(!duplicated(barcodes$ID)),] # isolate RM information from barcode file!


rm1 <- sub("(B...)_.*", "\\1", combos$ID)
rm2 <- sub("B..._(....)*", "\\1", combos$ID)

rm1 <- as.numeric(glumanda$rm[match(rm1, glumanda$ID)])
rm2 <- as.numeric(glumanda$rm[match(rm2, glumanda$ID)])

#data.frame(combos$ID, rm1, rm2)
